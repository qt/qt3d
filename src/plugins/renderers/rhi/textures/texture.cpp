...
RHITexture::TextureUpdateInfo RHITexture::createOrUpdateGLTexture()
{
    ...
    if (testDirtyFlag(SharedTextureId) || hasSharedTextureId) {
        ...
        if (!m_gl) {
            ...
            if (!m_gl->isStorageAllocated()) {
                qWarning() << "[Qt3DRender::RHITexture] failed to allocate texture";
                textureInfo.properties.status = QAbstractTexture::Error;
                return textureInfo;
            }
...
QOpenGLTexture *RHITexture::buildGLTexture()
{
...
void RHITexture::uploadGLTextureData()
{
...
void RHITexture::updateGLTextureParameters()
{
    const QAbstractTexture::Target actualTarget = m_properties.target;
