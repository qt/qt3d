...
public:
    ...
    RHIBuffer *glBufferForRenderBuffer(Buffer *buf);
    ...
    // Fences
    RHIFence fenceSync();
    void    clientWaitSync(RHIFence sync, GLuint64 nanoSecTimeout);
    void    waitSync(RHIFence sync);
    bool    wasSyncSignaled(RHIFence sync);
    void    deleteSync(RHIFence sync);
...
private:
    ...
    // Buffers
    HRHIBuffer createGLBufferFor(Buffer *buffer);
    void uploadDataToGLBuffer(Buffer *buffer, RHIBuffer *b, bool releaseBuffer = false);
    QByteArray downloadDataFromGLBuffer(Buffer *buffer, RHIBuffer *b);
    bool bindGLBuffer(RHIBuffer *buffer, RHIBuffer::Type type);
    ...
    {
        HRHIBuffer bufferHandle;
        RHIBuffer::Type attributeType;
        int location;
        GLint dataType;
        uint byteOffset;
        uint vertexSize;
    ...
    using VAOIndexAttribute = HRHIBuffer;
