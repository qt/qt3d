...
{
public:
    static void lock(RHITexture *tex)
    {
    ...
    static void unlock(RHITexture *tex)
    {
        if (!tex->isExternalRenderingEnabled())
            return;
        if (!s_lockHash.keys().contains(tex))
...
private:
    static QHash<RHITexture*, int> s_lockHash;
};

QHash<RHITexture*, int> TextureExtRendererLocker::s_lockHash = QHash<RHITexture*, int>();
...
void TextureSubmissionContext::deactivateTexture(RHITexture* tex)
{
    for (int u=0; u<m_activeTextures.size(); ++u) {
 ...
 */
int TextureSubmissionContext::assignUnitForTexture(RHITexture *tex)
{
