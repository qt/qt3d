...
RHIBuffer::Type attributeTypeToGLBufferType(QAttribute::AttributeType type)
{
    switch (type) {
...
bool SubmissionContext::setParameters(ShaderParameterPack &parameterPack)
{
    ...
    for (int i = 0; i < parameterPack.images().size(); ++i) {
        ...
        if (uniformValues.contains(namedTex.glslNameId)) {
            ...
            if (img != nullptr) {
                RHITexture *t = m_renderer->rhiResourceManagers()->rhiTextureManager()->lookupResource(img->textureId());
                if (t == nullptr) {
    ...
    for (const BlockToSSBO b : blockToSSBOs) {
        ...
        bindGLBuffer(ssbo, RHIBuffer::ShaderStorageBuffer);
    ...
    for (const BlockToUBO &b : blockToUBOs) {
        ...
        // Needed to avoid conflict where the buffer would already
        // be bound as a VertexArray
        bindGLBuffer(ubo, RHIBuffer::UniformBuffer);
                                       ...
                                       const ShaderAttribute *attributeDescription)
{
    ...
    const HRHIBuffer glBufferHandle = m_renderer->rhiResourceManagers()->rhiBufferManager()->lookupHandle(buffer->peerId());
    Q_ASSERT(!glBufferHandle.isNull());
    const RHIBuffer::Type attributeType = attributeTypeToGLBufferType(attribute->attributeType());
...
{
    RHIBuffer *buf = glBufferForRenderBuffer(buffer);
    if (!bindGLBuffer(buf, RHIBuffer::IndexBuffer))
        qCWarning(Backend) << Q_FUNC_INFO << "binding index buffer failed";

    // bound within the current VAO
    // Save this in the current emulated VAO
    if (m_currentVAO)
        m_currentVAO->saveIndexAttribute(m_renderer->rhiResourceManagers()->rhiBufferManager()->lookupHandle(buffer->peerId()));
}
...
void SubmissionContext::updateBuffer(Buffer *buffer)
{
    ...
    if (it != m_renderBufferHash.end())
        uploadDataToGLBuffer(buffer, m_renderer->rhiResourceManagers()->rhiBufferManager()->data(it.value()));
}
...
QByteArray SubmissionContext::downloadBufferContent(Buffer *buffer)
{
    const QHash<Qt3DCore::QNodeId, HRHIBuffer>::iterator it = m_renderBufferHash.find(buffer->peerId());
    if (it != m_renderBufferHash.end())
        return downloadDataFromGLBuffer(buffer, m_renderer->rhiResourceManagers()->rhiBufferManager()->data(it.value()));
    return QByteArray();
}
...
RHIBuffer *SubmissionContext::glBufferForRenderBuffer(Buffer *buf)
{
...
QByteArray SubmissionContext::downloadDataFromGLBuffer(Buffer *buffer, RHIBuffer *b)
{
    if (!bindGLBuffer(b, RHIBuffer::ArrayBuffer)) // We're downloading, the type doesn't matter here
        qCWarning(Io) << Q_FUNC_INFO << "buffer bind failed";
